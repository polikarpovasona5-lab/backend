...
class UserService:
    ...
    def create_user(db: Session, user_data: UserCreate) -> UserResponse:
        # Проверка возраста
        if user_data.age < 0:
            raise HTTPException(status_code=400, detail="Возраст должен быть положительным")
...
class PostService:
    ...
    def create_post(db: Session, post_data: PostCreate) -> PostResponse:
        ...
        db_post = PostRepository.create_post(db, post_data, PostService.CURRENT_USER_ID)
    ...
    @staticmethod
    def get_user_posts(db: Session, limit: Optional[int] = None) -> List[PostResponse]:
        # Получаем посты текущего пользователя напрямую (без репозитория)
        from sqlalchemy import select
        
        query = select(Post).where(Post.user_id == PostService.CURRENT_USER_ID).order_by(Post.created_at.desc())
